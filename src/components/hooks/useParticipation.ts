...
export const useParticipations = (postId: number, page: number = 0, size: number = 8) => {
 ...
 * 2) 단일 참여자 상태 조회
 */
export const useParticipationStatus = (postId: number, userId: number) => {
  ...
  return useMutation<void, Error, ParticipationMutationVars>({
    ...
    onSuccess: (_, vars) => {
      qc.invalidateQueries({
        queryKey: [BASE_KEY, vars.postId, vars.page, vars.size] as const,
        exact: false,
      });
    },
  });
...
export const useAcceptedParticipations = (postId: number, page: number = 0, size: number = 8) => {
  const queryKey = [BASE_KEY, postId, page, size] as const;

  return useQuery<PageResponse<RawParticipation>, Error>({
    queryKey,
    queryFn: () => fetchAcceptedParticipations({ postId, page, size }),
    enabled: postId > 0,
  });
};
