...
// 3) 참여 승인
export const approveParticipation = async (body: ApprovalRequest): Promise<void> => {
...
// 4) 참여 거절
export const rejectParticipation = async (body: ApprovalRequest): Promise<void> => {
...
export const requestParticipation = async (body: ParticipationRequest): Promise<void> => {
  await axiosInstance.post("/participants", {
    userId: body.userId,
    postId: body.postId,
  });
};

export const fetchAcceptedParticipations = async (
  opts: FetchParticipationOpts
): Promise<PageResponse<RawParticipation>> => {
  const { postId, page, size } = opts;
  const params: {
  ...
  const { data: resp } = await axiosInstance.get<{
    ...
    message: string;
    data: PageResponse<RawParticipation>;
  }>(`/participants/accepted/${postId}`, { params });
