...
import type { RawNotification, CreateNotificationRequest, ApiResponse } from "@/types/notification";
...
export const markNotificationAsRead = async (id: number, userId: number): Promise<void> => {
  await axiosInstance.patch<ApiResponse<string>>(`/notifications/${id}`, null, {
    params: { userId },
  });
};

// 3) 특정 유저의 알림 목록 조회
export const fetchNotifications = async (userId: number): Promise<RawNotification[]> => {
...
// 4) 특정 유저의 모든 알림 삭제
export const clearNotifications = async (userId: number): Promise<void> => {
