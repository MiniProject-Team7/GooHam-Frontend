...
export default function RepasswordPage() {
  ...
  const sendCode = () => {
    ...
    if (!validateEmail(email)) { setEmailError("invalid"); return; }
    setEmailError("");
    setCodeSent(true);
    startTimer();
  };
  ...
  const verifyCode = () => {
    ...
    if (timer <= 0) { setCodeError("expired"); return; }
    if (code !== DUMMY_CODE) { setCodeError("wrong"); return; }
    setCodeError("");
        ...
        <div className="min-h-screen flex items-center justify-center bg-[#FFCCCC]">
          <div className="flex items-center">
            ...
            <Card className="w-[500px] mt-[50px] mr-[100px] p-6 flex flex-col border-none shadow-none">
              <h1 className="text-center text-2xl font-bold mt-[20px] mb-[8px]">비밀번호 재설정 안내</h1>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex flex-col">
                  <label htmlFor="repass-email" className="text-sm font-bold mt-[-20px] mb-1 ">비밀번호를 재설정할 이메일</label>
                  <Input id="repass-email" type="email" placeholder="이메일을 입력해주세요"
                    value={email} disabled={false}
                    className={emailError ? "border-red-500 focus:border-red-500" : ""}
                    onChange={e => { setEmail(e.target.value); setEmailError(""); }}
                  />
                  {emailError === "required" && <p className="text-sm text-red-500 mt-1">이메일을 입력해 주세요.</p>}
                  {emailError === "invalid" && <p className="text-sm text-red-500 mt-1">이메일 양식을 확인해 주세요.</p>}
                </div>
                <Button className="block w-3/4 mx-auto " onClick={sendCode}>인증 번호 보내기</Button>

                <div className="relative mt-10 ">
                  <Input id="repass-code" placeholder="인증번호를 입력해주세요" value={code}
                    disabled={!codeSent || codeVerified}
                    className={codeError || codeVerified ? (codeError ? "border-red-500 focus:border-red-500" : "border-green-500 focus:border-green-500") : ""}
                    onChange={e => { setCode(e.target.value); setCodeError(""); }}
                  />
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-gray-500">{formatTime(timer)}</span>
                </div>
                {codeError === "required" && <p className="text-sm text-red-500">인증번호를 입력해 주세요.</p>}
                {codeError === "expired" && <p className="text-sm text-red-500">인증 제한 시간을 초과했습니다.</p>}
                {codeError === "wrong" && <p className="text-sm text-red-500">인증 번호가 틀렸습니다.</p>}
                {codeVerified && <p className="text-sm text-green-500">인증이 완료되었습니다.</p>}
                <Button className="block w-3/4 mx-auto" onClick={verifyCode} disabled={!codeSent || codeVerified}>인증하기</Button>

                <p className="text-sm font-bold mt-20 mb-2">새 비밀번호 설정</p>
                <div className="relative">
                  <Input id="repass-new" type={showNewPwd ? "text" : "password"} placeholder="비밀번호"
                    value={newPwd} disabled={!codeVerified}
                    className={((confirmPwdError && !newPwd) || pwdMismatch) ? "border-red-500 focus:border-red-500" : ""}
                    onChange={e => { setNewPwd(e.target.value); setConfirmPwdError(""); setPwdMismatch(false); }}
                  />
                ...
                <div className="relative">
                  <Input id="repass-confirm" type={showConfirmPwd ? "text" : "password"} placeholder="비밀번호 확인"
                    value={confirmPwd} disabled={!codeVerified}
                    className={((confirmPwdError && !confirmPwd) || pwdMismatch) ? "border-red-500 focus:border-red-500" : ""}
                ...
                {confirmPwdError === "required" && <p className="px-2 text-sm text-red-500">비밀번호를 입력해 주세요.</p>}
                {pwdMismatch && <p className="px-2 text-sm text-red-500">비밀번호가 일치하지 않습니다.</p>}
              </CardContent>

              <CardFooter className="mt-10 flex flex-col gap-3">
                <Button type="button" className="block w-3/4 mx-auto" onClick={changePassword} disabled={!codeVerified}>비밀번호 변경하기</Button>
                <Link href="/account/signin" className="block w-3/4 mx-auto">
                  <Button variant="outline" className="w-full border-gray-300 focus:border-gray-300">처음으로 돌아가기</Button>
                </Link>
              </CardFooter>
            </Card>
          </div>
